...
def main():
    ...
    collection_description = '这是与蒸馏相关的论文的集合'

    path1 = './database/collection.json'

    path2 = './database/doc_id.json'
    with open(path1, 'w') as f:
        for i in range(5):
            json.dump({f"{collection_name}_{i}":collection_description}, f)

    # temp = json.load(open(path2, 'r'))
    # print(temp)
    ...
    for article in artical_list:
        ...
        metadatas = [{"doc_id":article, "text_id":i, "collection":collection_name,
                       "description":f'与蒸馏相关的第{article}论文的第{i}个片段'} for i in range(len(docs))]
